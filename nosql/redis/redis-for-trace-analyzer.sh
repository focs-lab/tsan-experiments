...
function run {
    ...
    "redis-$OPTION/src/redis-server" redis.conf > /dev/null 2> /dev/shm/trace.txt &
    sleep 3
...
do
    ...
    if [[ "$OPTION" = "orig" ]]
    then
        { time USE_JEMALLOC=no CFLAGS="-g" make redis-server -j $(nproc) > /dev/null 2>&1; } |& tee -a ../../results/compile.txt
    elif [[ "$OPTION" = "tsan" ]]
    then
        { time SANITIZER=thread USE_JEMALLOC=no CFLAGS="-g" make redis-server -j $(nproc) > /dev/null 2>&1; } |& tee -a ../../results/compile.txt
    else
        ...
        { time SANITIZER=thread USE_JEMALLOC=no CFLAGS="$TSAN_FLAGS -g" make redis-server -j $(nproc) > /dev/null 2>&1; } |& tee -a ../../results/compile.txt
    fi
...
do
    ...
    run "results/$OPTION.txt" PING_INLINE 10000$L
    run "results/$OPTION.txt" PING_MBULK  10000$L
    run "results/$OPTION.txt" SET         500$L
    run "results/$OPTION.txt" GET         500$L
    run "results/$OPTION.txt" INCR        4000$L
    run "results/$OPTION.txt" LPUSH       150$L
    run "results/$OPTION.txt" RPUSH       150$L
    run "results/$OPTION.txt" LPOP        300$L
    run "results/$OPTION.txt" RPOP        300$L
    run "results/$OPTION.txt" SADD        250$L
    run "results/$OPTION.txt" HSET        500$L
    run "results/$OPTION.txt" SPOP        300$L
    run "results/$OPTION.txt" ZADD        250$L
    run "results/$OPTION.txt" ZPOPMIN     5000$L
    run "results/$OPTION.txt" LPUSH       150$L
    run "results/$OPTION.txt" LRANGE_100  1$L
    run "results/$OPTION.txt" LRANGE_300  1$L
    run "results/$OPTION.txt" LRANGE_500  1$L
    run "results/$OPTION.txt" LRANGE_600  1$L
    run "results/$OPTION.txt" MSET        100$L
