...
int WALTHREAD1_NTHREAD = 10;
int WALTHREAD3_NTHREAD = 6;
...
int main(int argc, char **argv){
  ...
  ** command-line and bail out with the usage message.  */
  for(iArg=1; iArg<argc; iArg++){
    ...
    /* Enabling specify number of threads */
    if( strcmp(zArg, "--w1-threads") == 0 ){
      if( ++iArg < argc ){
        WALTHREAD1_NTHREAD = atoi(argv[iArg]);
        continue;
      } else {
        fprintf(stderr, "Error: --w1-threads requires a number.\n");
        return 1;
      }
    }
    if( strcmp(zArg, "--w3-threads") == 0 ){
      if( ++iArg < argc ){
        WALTHREAD3_NTHREAD = atoi(argv[iArg]);
        continue;
      } else {
        fprintf(stderr, "Error: --w3-threads requires a number.\n");
        return 1;
      }
    }
