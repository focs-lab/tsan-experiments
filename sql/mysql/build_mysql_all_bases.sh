...
BUILDTYPELIST="${!CONFIG_DETAILS[@]} tsan-dom-ea-lo-st-swmr tsan-dom_peeling-ea-lo-st-swmr"
...
log "Starting MySQL build for all configurations..."

# 3. Iterate over all configurations
for config_name in $BUILDTYPELIST; do
	log "===== Building configuration: $config_name ====="

	if [ -f "mysql-$config_name/bin/mysqld" ]; then
		log "Skipping $config_name, binary already exists."
		continue
	fi

	if [ -d "mysql-$config_name" ]; then
		log "Moving directory 'mysql-$config_name' without bin/mysqld inside to 'partial-mysql-$config_name'..."
		mv "mysql-$config_name" "partial-mysql-$config_name"
	fi

	# Rename the temporary TSan directory before the build
	rename_dir_with_suffix "$TSAN_TMP_DIR" "${TSAN_TMP_DIR}_mysql_old"
	mkdir -p "$TSAN_TMP_DIR"

	# 4. Run the build and measure the time
	start_time=$SECONDS
	if ! ./build_mysql.sh "$config_name"; then
		log "Error building configuration: $config_name. Moving to failed-mysql-$config_name"
		[ -d "failed-mysql-$config_name" ] && rm -r "failed-mysql-$config_name"
		mv "mysql-$config_name" "failed-mysql-$config_name"
		exit 1
	fi
...
log "Building all MySQL configurations completed. All results are in $RESULTS_DIR."
