...
P5_STAGE_B_EXTRA="tsan-st tsan-stmt tsan-swmr tsan-lo tsan-ea tsan-dom tsan-dom_peeling tsan-sound-wp"
...
p5_configs_for() { local app=$1 set=$2 out=""; for c in $set; do case "$c" in *-wp) p5_has_wp "$app" || continue;; esac; out="$out $c"; done; echo $out; }
