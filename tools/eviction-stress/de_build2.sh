...
for c in tsan tsan-sound tsan-dom tsan-dom_peeling-ea-lo-st-swmr; do
  ...
  "$TSAN_CC" $flags -S -emit-llvm -o "$OUT/de_stress2.$c.ll" de_stress2.c
  "$TSAN_CC" $flags -o "$OUT/de_stress2.$c" de_stress2.c
  na=$(awk '/^define.*@a_body/,/^}/' "$OUT/de_stress2.$c.ll" | grep -c "@__tsan_write1" || true)
  nb=$(awk '/^define.*@burst_shared_on/,/^}/' "$OUT/de_stress2.$c.ll" | grep -c "@__tsan_\(read\|write\)" || true)
